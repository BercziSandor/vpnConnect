...
Func connect()
	...
	While $i < 10
		...
		; msg("Text on window: " & $txt )
		If StringInStr($txt, "Login using", $STR_CASESENSE ) Then
			ExitLoop
			msg("connect(): 'Login using' appeared, exit waiting.")
		Else
			msg("connect(): Still waiting for text 'Login using' on Jabber window...")
		EndIf
		$i = $i + 1
		Sleep(1000)
	WEnd
