...
Func loadPWs()
	...
	if FileExists($iniFile) Then
		...
		EndIf
	Else
		$pw1 = InputBox( "Enter password", "Please type the password for TIKS card.",     "", "*" )
		$pw2 = InputBox( "Enter password", "Please type the password for Windows login.", "", "*" )
		$pass = InputBox( "Enter password", "Please type the password for the ini file.",  "", "*" )
		$data_decr = StringToBinary( $pw1 & $sep & $pw2 )
...
Func connect()
	...
	if $vpnClientMode = $VPN_GRAPHICAL Then
		...
		$handleCiscoAnyConnect = u_activateWindow($winTitle)

		; 1: Standard method
		; u_clickOnButtonOnWindow( $winTitle, "Button1" )

		; 2: Keyboard
		u_SendToWindow($handleCiscoAnyConnect, "{TAB}{TAB}{TAB}{ENTER}")
	...
	if 1 >= 2 Then
		; get_VPN_state() is not ok.
		msg("Waiting for 90 seconds...")
		Sleep(90000)
	Else
...
Func u_activateWindow($winTitle)
	msg( "u_activateWindow(): Waiting for window '" & $winTitle & "'" )

	Local $hTimer = TimerInit()

	While 1
		If (WinActivate($winTitle) <> 0) Or (WinActive($winTitle) <> 0) Then
	...
	$winHandle = WinWaitActive($winTitle)
	If $winHandle = 0 Then
